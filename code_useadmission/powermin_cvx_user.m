function [Qsolution,feasible] = powermin_cvx_user(params)
...
for i=1:1:length(K_activeset)  
    ...
    for k=1:K_activeset(i) 
        ...
        (H(:,m,K_index(sum(K_activeset(1:i-1))+k)-sum(K_set(1:m-1)))'*G*H(:,m,K_index(sum(K_activeset(1:i-1))+k)-sum(K_set(1:m-1)))-r_set(i)*delta_set(i)^2)==hermitian_semidefinite(1);
    end
end
cvx_end
