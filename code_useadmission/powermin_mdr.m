function [Vsolution,feasible] = powermin_mdr(params)
...
variable S(2,2,sum(K_activeset)) hermitian semidefinite;  %%slack variables for objective function
...
minimize (epsilon*(weight'*t)+(1-epsilon)*sum(s))  % group sparsity inducing minimization
subject to
...
for i=1:1:length(K_activeset)  
    ...
    for k=1:K_activeset(i) 
        ...
        (H(:,m,K_index(sum(K_activeset(1:i-1))+k)-sum(K_set(1:m-1)))'*G*H(:,m,K_index(sum(K_activeset(1:i-1))+k)-sum(K_set(1:m-1)))-r_set(i)*delta_set(i)^2+d^(-1)*trace(coe*S(:,:,sum(K_activeset(1:i-1))+k)))==hermitian_semidefinite(1);
    end
end
cvx_end
